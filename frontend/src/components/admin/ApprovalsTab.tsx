...
import { confirmReviewItem, fetchReviewQueue, rejectReviewItem } from "../../api/sanctions";
...
export function ApprovalsTab() {
  ...
  return (
    <div className="space-y-5">
      ...
      {msg && (
        <p className="mono rounded-lg border border-teal-200 bg-teal-50 px-3 py-2 text-xs text-[var(--accent-deep)]">
          {msg}
        </p>
      )}
        ...
        {!loading && items.length > 0 && (
          <div className="thin-scroll overflow-auto">
            <table className="admin-table min-w-[760px]">
              <thead>
                <tr>
                  <th>ID</th><th>IMO</th><th>OS Entity</th><th>Status</th><th>Confidence</th><th>Actions</th>
                </tr>
              </thead>
              <tbody>
                {items.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td className="max-w-[220px] truncate font-medium text-[var(--text-primary)]">
                      {item.os_entity_id}
                    </td>
                    <td>
