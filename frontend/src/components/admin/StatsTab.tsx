...
const PIE_COLORS = [OCEAN_BLUE, BLUE, AMBER, RED, "#8a93a3", "#7cc8ba", "#6e9df2", "#e0aa70"];
    ...
    <div className="space-y-6">
      ...
      <section className="grid gap-4 md:grid-cols-3">
        {[
          ...
          <div key={s.label} className="admin-card-enter p-5" style={{ animationDelay: `${i * 50}ms` }}>
            ...
            <div className="flex flex-col gap-6 sm:flex-row sm:items-center">
              <ResponsiveContainer width={220} height={220}>
                <PieChart>
                  ...
                  <Tooltip contentStyle={TOOLTIP_STYLE} formatter={(v) => [v, "vessels"]} />
                </PieChart>
              </ResponsiveContainer>
              <ul className="space-y-1 text-xs">
                {vesselTypes.slice(0, 8).map((vt, i) => (
                  <li key={vt.vessel_type} className="flex items-center gap-2">
                    <span className="inline-block h-2.5 w-2.5 rounded-sm" style={{ background: PIE_COLORS[i % 8] }} />
        ...
        {topRisk.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={topRisk} layout="vertical" barSize={12}>
              ...
              <YAxis type="category" dataKey="name" width={130} tick={{ fontSize: 10, fill: TICK }} />
              <Tooltip contentStyle={TOOLTIP_STYLE} formatter={(v) => [Number(v).toFixed(1), "risk"]} />
              <Bar dataKey="score" radius={[0, 6, 6, 0]} isAnimationActive>
                {topRisk.map((row, i) => (
                  <Cell key={i} fill={row.score >= 75 ? RED : row.score >= 40 ? AMBER : OCEAN_BLUE} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        ...
        {entities.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={entities.slice(0, 15)} barSize={16}>
              <CartesianGrid strokeDasharray="3 3" stroke={GRID} />
              <XAxis dataKey="entity_text" tick={{ fontSize: 9, fill: TICK }} angle={-30} textAnchor="end" height={50} />
              <YAxis tick={{ fontSize: 10, fill: TICK }} />
              <Tooltip contentStyle={TOOLTIP_STYLE} />
