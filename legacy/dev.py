#!/usr/bin/env python3
"""OceansX V2 — Development Control Panel · python dev.py"""
...
def _read_backend_env(name: str) -> str:
    """Read one simple KEY=value from backend/.env without importing app config."""
...
def _frontend_env() -> dict[str, str]:
    ...
    env = {
        # Most frontend clients use /api through Vite's proxy. This keeps fetch-based
        # clients aligned if the app is opened from the Vite dev server.
...
def _preflight() -> bool:
    ok = True
    checks = [
        (BACKEND / ".env", "backend/.env missing"),
        (UVICORN, "backend venv missing uvicorn — cd backend && python -m venv .venv && pip install -e ."),
        (ALEMBIC, "backend venv missing alembic — cd backend && pip install -e ."),
        (FRONTEND / "package.json", "frontend/package.json missing"),
        (FRONTEND / "node_modules", "frontend dependencies missing — cd frontend && npm install"),
    ...
    if not _read_backend_env("ADMIN_TOKEN") and not os.environ.get("VITE_ADMIN_TOKEN"):
        _log("WARNING: ADMIN_TOKEN missing; admin dashboard requests will return 401")
    return ok
...
def _brew_pg_version() -> str | None:
    """Return the highest installed Homebrew PostgreSQL version, or None."""
    for ver in ("17", "16", "15", "14"):
        try:
            r = subprocess.run(
                ["brew", "list", f"postgresql@{ver}"], capture_output=True, timeout=4
            )
            if r.returncode == 0:
                return ver
        except Exception:
...
def _has_brew_pg() -> bool:
    return _brew_pg_version() is not None
...
def _stop_backend() -> None:
    ...
    if proc:
        ...
        _log("Backend is running outside this panel; leaving it online")
        with _lock:
            s.status = St.RUNNING
        return
    with _lock:
...
def _start_frontend() -> None:
    ...
    if not VITE_BIN.exists():
        with _lock:
            s.status = St.ERROR
        _log("ERROR: frontend dependencies missing — cd frontend && npm install")
        return
    proc = subprocess.Popen(
...
def _stop_frontend() -> None:
    ...
    if proc:
        ...
        _log("Frontend is running outside this panel; leaving it online")
        with _lock:
            s.status = St.RUNNING
        return
    with _lock:
...
def _migrate() -> None:
    ...
    if not ALEMBIC.exists():
        ...
        return
    if not HEALTH["db"]():
        _log("ERROR: database is not reachable; migrations skipped")
        return
    proc = subprocess.Popen(
...
BRAND = [
    " ┌──────────────────────────────────────────────────────────┐",
    " │   ≋  O C E A N S X  ·  V 2  ─  Control Panel  ≋        │",
    " └──────────────────────────────────────────────────────────┘",
]
...
def _draw_log_view(scr, key: str, scroll: int, follow: bool) -> tuple[int, bool]:
    ...
    # Footer
    _hline(scr, h - 2, "─", cy)
    info = f"  {scroll + 1}-{min(scroll + visible, max(total, 1))}/{max(total, 0)}    ↑↓ PgUp/PgDn scroll    f follow    q/ESC back"
...
def _main(scr) -> None:
    ...
    bg = -1 if _use_default_colors else curses.COLOR_BLACK
        ...
        for s in svcs.values():
            if s.proc:
                try:
                    ...
                    pass
    print("OceansX Control Panel exited.")
