"""Sanctions API endpoints — Phase 4b.

Provides:
  GET  /api/sanctions/vessel/{imo}       — sanctions matches for a vessel
  GET  /api/sanctions/review-queue       — admin review queue (protected)
...
async def get_vessel_sanctions(
    ...
    """Return all sanctions matches for a vessel (auto_confirmed + confirmed + pending)."""
    ...
    for match in matches:
        # Look up entity details from raw table
...
async def get_review_queue(
    ...
    """Admin: list review queue items. Requires Bearer admin token."""
...
async def confirm_review_item(
    ...
    """Admin: confirm a review queue item → updates sanctions_match to confirmed."""
    ...
    # Upsert the corresponding sanctions_match row as confirmed
    match_result = await db.execute(
    ...
    await db.commit()
    logger.info("review_queue: item=%d confirmed (imo=%d entity=%s)", item_id, item.imo, item.os_entity_id)
...
async def reject_review_item(
    ...
    """Admin: reject a review queue item → updates sanctions_match to rejected."""
    ...
    await db.commit()
    logger.info("review_queue: item=%d rejected (imo=%d entity=%s)", item_id, item.imo, item.os_entity_id)
