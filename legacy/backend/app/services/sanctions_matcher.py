...
ADR-0005: IMO-exact is the ONLY match_method that may produce status='auto_confirmed'.
All other match methods (name_flag_fuzzy, name_fuzzy, org_link) → status='pending',
written to agent_review_queue. This rule is enforced here in code AND by a database
CHECK constraint (ck_sanctions_match_auto_confirm_imo_only) so it cannot be bypassed.

Referents: when an entity is matched, the matcher checks both the entity's current id
and all values listed in entity['referents'] (for ID merges/splits over time).
"""
...
async def _record_match(
    ...
    """Upsert a sanctions_match row and append to history."""
    now = utc_now()

    # Fetch existing match if any
    ...
    if existing is not None:
        ...
        if old_status == status:
            return existing
        # Only escalate, never downgrade auto_confirmed
        if existing.status == "auto_confirmed":
            return existing
...
async def _enqueue_review(
    ...
    """Write to agent_review_queue for human review."""
    session.add(
        AgentReviewQueue(
            imo=imo,
            os_entity_id=os_entity_id,
...
async def _update_vessel_sanctions_status(
    session: AsyncSession,
    imo: int,
    new_status: str,
) -> None:
    await session.execute(
        ...
        .values(current_sanctions_status=new_status)
    )
...
async def match_vessel_entity(
    ...
    """Attempt to match a Vessel FtM entity against known vessels.

    IMO exact match → auto_confirmed, vessel.current_sanctions_status → 'sanctioned'.
    All other matches → pending in agent_review_queue.
    ...
    # All IDs this entity has ever had (current + merged)
    all_entity_ids: list[str] = [os_entity_id] + list(entity.get("referents", []))
    ...
    for imo_raw in imo_strings:
        ...
        # Check vessel exists in our DB
        vessel_result = await session.execute(
            select(Vessel.imo).where(Vessel.imo == imo)
        )
        if vessel_result.scalar_one_or_none() is None:
            continue

        # IMO-exact match → AUTO-CONFIRM (ADR-0005)
        await _record_match(
            session,
            imo=imo,
            os_entity_id=os_entity_id,
        ...
        # Write sanctions_listing rows per dataset
        ...
        logger.info(
            "sanctions_matcher: IMO-exact auto_confirmed imo=%d entity=%s",
            imo, os_entity_id,
        )
...
async def run_matcher(
    ...
    Non-vessel entities are skipped — org-link matching is deferred to Phase 4b.
    ...
    for entity in entities:
        ...
        try:
            ...
            errors += 1
            logger.warning(
                "sanctions_matcher: error for entity=%s: %s",
                entity.get("id", "?"),
                exc,
            )
