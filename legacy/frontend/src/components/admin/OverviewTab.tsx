  ...
  return (
    <div className="space-y-6">
      ...
      <section className="admin-card-enter p-5">
        ...
        {actionMsg && (
          <p className="mono mt-4 rounded-lg border border-teal-200 bg-teal-50 px-3 py-2 text-xs text-[var(--accent-deep)]">{actionMsg}</p>
        )}
      </section>
        ...
        <section className="admin-card-enter overflow-hidden" style={{ animationDelay: "100ms" }}>
          ...
          {depAudit.length === 0 ? (
            ...
            <div className="thin-scroll max-h-[360px] overflow-auto">
              <table className="admin-table">
                <thead>
                  <tr><th>Ecosystem</th><th>High/Critical</th><th>Total</th><th>Run at</th></tr>
                </thead>
                <tbody>
                  {depAudit.map((d) => (
                    <tr key={d.id}>
                      <td className="font-medium text-[var(--text-primary)]">{d.ecosystem}</td>
                      <td className={d.high_critical_count > 0 ? "font-semibold text-red-700" : "font-semibold text-teal-700"}>{d.high_critical_count}</td>
                      <td>{d.total_count}</td>
                      <td className="mono text-[11px] text-[var(--text-muted)]">{new Date(d.audited_at).toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <section className="admin-card-enter overflow-hidden" style={{ animationDelay: "140ms" }}>
        ...
        {auditLog.length === 0 ? (
          ...
          <div className="thin-scroll max-h-[440px] overflow-auto">
            <table className="admin-table">
              <thead>
                <tr><th>Time</th><th>Actor</th><th>Action</th><th>Severity</th></tr>
              </thead>
              <tbody>
                {auditLog.map((e) => (
                  <tr key={e.id}>
                    <td className="mono whitespace-nowrap text-[11px] text-[var(--text-muted)]">{new Date(e.occurred_at).toLocaleString()}</td>
                    <td>{e.actor}</td>
                    <td className="font-medium text-[var(--text-primary)]">{e.action}</td>
                    <td className={e.severity === "warning" ? "font-semibold text-amber-700" : e.severity === "error" ? "font-semibold text-red-700" : "text-[var(--text-muted)]"}>{e.severity}</td>
                  </tr>
                ))}
              </tbody>
            </table>
