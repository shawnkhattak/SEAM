...
async def ingest_article(
    ...
    """Insert a single article if the URL has not been seen before. Returns None on duplicate."""
...
async def get_or_create_feed(session: AsyncSession, slot: int, name: str, url: str | None = None) -> int:
    ...
    result = await session.execute(select(NewsFeed.id).where(NewsFeed.feed_slot == slot))
...
async def get_recent_news(
    ...
    if entity_type and entity_ref_id is not None:
        ...
        stmt = select(NewsItem).order_by(desc(NewsItem.published_at_utc)).limit(limit)
...
async def prune_old_news(session: AsyncSession, days: int = 90) -> int:
    ...
    result = await session.execute(delete(NewsItem).where(NewsItem.published_at_utc < cutoff))
