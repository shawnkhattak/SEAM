...
then sets vessel.is_shadow_fleet accordingly. Run nightly after opensanctions ingest.
"""
...
async def refresh_shadow_fleet_flags(session: AsyncSession) -> dict[str, int]:
    ...
    Vessels with active 'mare.shadow' topic → is_shadow_fleet=True; all others → False.
    ...
    if shadow_imos:
        res = await session.execute(
            update(Vessel).where(Vessel.imo.in_(shadow_imos)).values(is_shadow_fleet=True)
        )
