...
# ---------------------------------------------------------------------------
# Config CRUD (app_config table via ConfigCache)
# ---------------------------------------------------------------------------
...
async def list_config_keys(
    ...
    """List all config keys. Secret values are masked."""
    ...
    return [
        {
            "key": r.key,
            "value": "***" if r.is_secret else r.value_plain,
...
@router.put("/config/{key}", status_code=200)
...
async def set_config_key(
    ...
    """Set a config key. Body: {value, is_secret?, description?}"""
    from app.models import AppConfig
    from app.utils.timezone import utc_now

    body = await request.json()
    value: str = body.get("value", "")
    is_secret: bool = bool(body.get("is_secret", False))
    ...
    if row is None:
        row = AppConfig(
            key=key,
            value_plain=value if not is_secret else None,
        ...
        row.value_plain = value if not is_secret else None
        row.updated_at = now
    ...
    await append_audit_log(
        ...
        target_id=key,
        detail={"is_secret": is_secret},
    )
    await db.commit()

    # Invalidate ConfigCache
    if hasattr(request.app.state, "config_cache"):
        request.app.state.config_cache.invalidate(key)
...
@router.get("/enrichment-queue", status_code=200)
@limiter.limit("30/minute")
async def enrichment_queue_stats(
    request: Request,
    db: AsyncSession = Depends(get_db),
    _: str = Depends(require_admin),
) -> dict[str, Any]:
    """Summary stats for the vessel enrichment queue."""
    result = await db.execute(
        text(
            "SELECT "
